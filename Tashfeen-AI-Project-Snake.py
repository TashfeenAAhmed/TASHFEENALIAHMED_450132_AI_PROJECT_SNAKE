...
class SnakeGame:
    def __init__(self, master):
        ...
        self.obstacles = [(100, 160), (180, 180), (240, 540), (140, 460), (420, 20), (580, 560)]
    ...
    def create_food(self):
        ...
        food = self.canvas.create_rectangle(x, y, x + 20, y + 20, fill="yellow")
        return food
    ...
    def move_enemy_snake(self):
        if not self.enemy_out:
            ...
            if abs(distance_x) > abs(distance_y):
                # Check if the food is to the right or left
                if distance_x > 0 and not self.is_obstacle_in_direction("Right"):
                    self.enemy_direction = "Right"
                elif distance_x < 0 and not self.is_obstacle_in_direction("Left"):
                    self.enemy_direction = "Left"
            else:
                # Check if the food is above or below
                if distance_y > 0 and not self.is_obstacle_in_direction("Down"):
                    self.enemy_direction = "Down"
                elif distance_y < 0 and not self.is_obstacle_in_direction("Up"):
                    self.enemy_direction = "Up"

                # Additional checks for southeast and southwest scenarios
                if distance_x > 0 and distance_y > 0 and not self.is_obstacle_in_direction("DownRight"):
                    self.enemy_direction = "DownRight"
                elif distance_x < 0 < distance_y and not self.is_obstacle_in_direction("DownLeft"):
                    self.enemy_direction = "DownLeft"
    ...
    def is_obstacle_in_direction(self, direction):
        head = self.enemy_snake[0]

        if direction == "Right" and not (head[0] + 20 >= 600 or (head[0] + 20, head[1]) in self.enemy_snake):
            return False
        elif direction == "Left" and not (head[0] - 20 < 0 or (head[0] - 20, head[1]) in self.enemy_snake):
            return False
        elif direction == "Up" and not (head[1] - 20 < 0 or (head[0], head[1] - 20) in self.enemy_snake):
            return False
        elif direction == "Down" and not (head[1] + 20 >= 600 or (head[0], head[1] + 20) in self.enemy_snake):
            return False

        return True

    def reverse_direction(self, direction):
        # Helper method to reverse the direction
        if direction == "Right":
            return "Left"
        elif direction == "Left":
            return "Right"
    ...
    def update(self):
        ...
        for segment in self.snake:
            self.canvas.create_rectangle(segment[0], segment[1], segment[0] + 20,
                                         segment[1] + 20, fill="blue", tags="snake")
        if not self.enemy_out:
            for segment in self.enemy_snake:
                self.canvas.create_rectangle(segment[0], segment[1], segment[0] + 20,
                                             segment[1] + 20, fill="red", tags="enemy_snake")
        ...
        if enemy_head[0] == food_coords[0] and enemy_head[1] == food_coords[1]:
            ...
            self.enemy_score += 1
            self.enemy_score_label.config(text=f"Your Score: {self.enemy_score}")  # Update the score label
            if self.food:
                self.canvas.delete(self.food)
                self.food = None
            self.food = self.create_food()
        ...
        self.master.after(175, self.update)  # to increase speed decrease the value,
